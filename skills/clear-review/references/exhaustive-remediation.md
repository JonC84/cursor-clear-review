...
4. **Continuous:** if clear, immediately start the next unchecked wave. **Pause:** if clear, print Quick manual checks, then STOP for `continue` / `skip` / `abort` (user may fix in-session if a check fails).
